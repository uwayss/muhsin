...
type CreateDeedScreenNavigationProp = NativeStackNavigationProp<HomeStackParamList, 'CreateDeed'>;
...
const CreateDeedScreen = () => {
  ...
  const { draftDeed, initializeDraftDeed, updateDraftDeed, clearDraftDeed, saveDraftDeed } =
    useAppStore();
  ...
  const handleSave = () => {
    if (!draftDeed?.name?.trim()) {
      Alert.alert(i18n.t('alerts.missingNameTitle'), i18n.t('alerts.missingNameMessage'));
      return;
    }
    saveDraftDeed();
    navigation.goBack();
  ...
  const handleSelectIcon = (icon: React.ComponentProps<typeof MaterialCommunityIcons>['name']) => {
  ...
  const screenTitle = isEditMode ? i18n.t('screens.editDeed') : i18n.t('screens.createDeed');
      ...
      <Screen
        ...
        renderRightAction={() => (
          <TouchableOpacity onPress={handleSave}>
            <ThemedText style={styles.saveText}>{i18n.t('deeds.save')}</ThemedText>
          </TouchableOpacity>
        )}>
        <View style={styles.formContainer}>
          <Box style={styles.inputRow}>
            <TouchableOpacity style={styles.iconPicker} onPress={() => setIconModalVisible(true)}>
              <MaterialCommunityIcons name={draftDeed.icon} size={32} color={theme.colors.text} />
            </TouchableOpacity>
            <ThemedTextInput
          ...
          <ThemedText style={styles.sectionHeader}>{i18n.t('deeds.configuration')}</ThemedText>
          <ConfigRow
            icon="calendar-sync"
...
const ConfigRow = ({
  ...
  return (
    <TouchableOpacity style={styles.configRow} onPress={onPress}>
      <MaterialCommunityIcons name={icon} size={24} color={theme.colors.textSecondary} />
