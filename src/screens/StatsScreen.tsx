...
const StatsScreen = () => {
  ...
  const goalDeeds = useMemo(() => deeds.filter((d) => d.goal && !d.isCore), [deeds]);
  ...
  const graphData = useMemo(() => {
    ...
    const relevantLogs = logs.filter((log) => new Date(log.date) >= sixtyDaysAgo);
      ...
      const filtered =
        interval === 'all'
          ...
          : logs.filter((log) => {
              ...
              return logDate >= dateRangeForFilter.start && logDate <= dateRangeForFilter.end;
            });

      const prayerLogs = filtered.filter((log) => PRAYER_DEED_IDS.includes(log.deedId));
  ...
  const renderContent = () => {
    ...
    return (
      <>
        <SegmentedControl options={segmentedOptions} selected={interval} onSelect={setInterval} />
        <ScrollView
          contentContainerStyle={styles.scrollContent}
          showsVerticalScrollIndicator={false}>
          <ThemedText style={styles.sectionTitle}>{i18n.t('stats.prayerPerformance')}</ThemedText>
          ...
          {goalDeeds.length > 0 && (
            <View>
              <ThemedText style={styles.sectionTitle}>{i18n.t('stats.goalProgress')}</ThemedText>
              <View style={styles.card}>
                {goalDeeds.map((deed) => (
                  <GoalStatsRow
                    key={deed.id}
                    deed={deed}
                    logs={displayData.filteredLogs.filter((l) => l.deedId === deed.id)}
                  />
                ))}
              </View>
            </View>
          ...
          <ThemedText style={styles.sectionTitle}>{i18n.t('stats.statsByDeed')}</ThemedText>
          <ExpandableSection title={i18n.t('stats.fardPrayers')}>
            {prayerDeeds.map((deed) => (
              <DeedStatsRow key={deed.id} deed={deed} logs={displayData.filteredLogs} />
            ))}
          </ExpandableSection>
        </ScrollView>
      </>
  ...
  return (
    <Screen title={i18n.t('screens.stats')}>
      <ThemedText style={styles.sectionTitle}>{i18n.t('stats.activity')}</ThemedText>
      <ActivityGraph deeds={prayerDeeds} logs={graphData.logs} dateRange={graphData.dateRange} />
      {renderContent()}
    </Screen>
  );
};
